...
static void make_unique_folder(const char *base, char *folder, size_t folder_size)
{
    ...
    folder[folder_size - 1] = '\0';
    snprintf(candidate, sizeof(candidate), "%s/%s", SWITCH_WHDLOAD_ROOT, folder);
    int is_dir = 0;
...
static int lha_read1(LhaNativeDecoder *dec)
{
    ...
    if (dec->in_pos >= dec->in_len) {
        ...
        int got = (int)read(dec->in_fd, dec->in_buf, want);
        if (got <= 0)
            return -1;
        dec->in_pos = 0;
        dec->in_len = got;
...
static int lha_flush_output(int out_fd, char *buffer, int *buffer_pos)
{
    if (*buffer_pos <= 0)
        return 1;
    int written = write(out_fd, buffer, *buffer_pos);
...
static int lha_decode_file(int in_fd, int out_fd, uint32_t comp_size, uint32_t orig_size)
{
...
extern "C" void switch_gui_draw_progress(const char *title, const char *subtitle, float fraction, const char *item_name);
...
static int lha_read_header(int in_fd, LhaHeaderInfo *h)
{
    uint8_t hsize = 0;
    uint8_t hchk = 0;
    uint32_t time = 0;
    ...
    if (read(in_fd, &hsize, 1) != 1 || hsize == 0)
        return 0;
    if (read(in_fd, &hchk, 1) != 1)
        return 0;
    memset(h, 0, sizeof(*h));
    if (read(in_fd, h->method, 5) != 5)
        return 0;
    h->method[5] = '\0';
    if (read(in_fd, &h->comp_size, 4) != 4 ||
        read(in_fd, &h->orig_size, 4) != 4 ||
        read(in_fd, &time, 4) != 4 ||
        read(in_fd, &attr, 1) != 1 ||
        read(in_fd, &h->level, 1) != 1)
        return 0;

    if (h->level == 0 || h->level == 1) {
        uint8_t nlen = 0;
        if (read(in_fd, &nlen, 1) != 1)
            return 0;
        if (read(in_fd, h->name, nlen) != nlen)
            return 0;
        h->name[nlen] = '\0';
        if (read(in_fd, &crc, 2) != 2)
            return 0;
        if (h->level == 1) {
            uint8_t os = 0;
            if (read(in_fd, &os, 1) != 1)
                return 0;
            uint16_t ext_size = 0;
            while (read(in_fd, &ext_size, 2) == 2 && ext_size > 0) {
                h->ext_total += ext_size;
                uint8_t ext_type = 0;
                if (read(in_fd, &ext_type, 1) != 1)
                    return 0;
                if (ext_type == 0x02 && ext_size > 3) {
                    int dlen = ext_size - 3;
                    int read_len = dlen > 510 ? 510 : dlen;
                    if (read(in_fd, h->dirname, read_len) != read_len)
                        return 0;
                    h->dirname[read_len] = '\0';
                    for (int i = 0; i < read_len; i++)
                        if (h->dirname[i] == (char)0xFF) h->dirname[i] = '/';
                    if (dlen > read_len)
                        lseek(in_fd, dlen - read_len, SEEK_CUR);
                } else if (ext_type == 0x01 && ext_size > 3) {
                    int flen = ext_size - 3;
                    int read_len = flen > 510 ? 510 : flen;
                    if (read(in_fd, h->name, read_len) != read_len)
                        return 0;
                    h->name[read_len] = '\0';
                    if (flen > read_len)
                        lseek(in_fd, flen - read_len, SEEK_CUR);
                } else if (ext_size > 3) {
                    lseek(in_fd, ext_size - 3, SEEK_CUR);
                }
            }
        }
    }
...
{
    int in_fd = open(archive_path, O_RDONLY, 0);
    if (in_fd < 0) return 0;
    ...
    while (lha_read_header(in_fd, &h)) {
        ...
        lseek(in_fd, actual_comp, SEEK_CUR);
    }
...
static int extract_native_lha(const char *archive_path, const char *destination_root)
{
    ...
    while (lha_read_header(in_fd, &h)) {
        ...
        if (!make_safe_relative(raw_path, relative, sizeof(relative))) {
            ...
            lseek(in_fd, skip_bytes, SEEK_CUR);
            continue;
        }
        ...
        switch_gui_draw_progress("Extracting WHDLoad Game...", sub, fraction, raw_path);

        off_t data_start = lseek(in_fd, 0, SEEK_CUR);
        if (strcmp(h.method, "-lh0-") == 0) {
            int out_fd = open(output_path, O_WRONLY | O_CREAT | O_TRUNC, 0777);
            if (out_fd >= 0) {
                char buf[4096];
                uint32_t remaining = h.orig_size;
                while (remaining > 0) {
                    uint32_t to_read = remaining < sizeof(buf) ? remaining : sizeof(buf);
                    int r = read(in_fd, buf, to_read);
                    if (r <= 0) break;
                    write(out_fd, buf, r);
                    remaining -= r;
                }
        ...
        lseek(in_fd, data_start + actual_comp, SEEK_SET);
    }
...
static la_ssize_t custom_archive_read(struct archive *a, void *client_data, const void **buff)
{
    ...
    *buff = ctx->buffer;
    int bytes = read(ctx->fd, ctx->buffer, sizeof(ctx->buffer));
    if (bytes < 0) return -1;
...
int switch_whdload_install_lha(const char *archive_path, char *installed_path, size_t installed_path_size)
{
    ...
    snprintf(destination_root, sizeof(destination_root), "%s/%s", SWITCH_WHDLOAD_ROOT, folder);
    ...
    ctx->fd = open(archive_path, O_RDONLY, 0);
    if (ctx->fd < 0) {
...
static void deploy_whdload_file(const char *rel_subpath, const char *dst)
{
    ...
    for (int i = 0; prefixes[i]; i++) {
        char src[512];
        snprintf(src, sizeof(src), "%s/%s", prefixes[i], rel_subpath);
        struct stat st;
...
{
    ensure_directory(SWITCH_WHDLOAD_ROOT);
    ensure_directory(SWITCH_WHDLOAD_ROOT "/C");
    ...
    deploy_whdload_file("C/DIC", SWITCH_WHDLOAD_ROOT "/C/DIC");
    deploy_whdload_file("C/Patcher", SWITCH_WHDLOAD_ROOT "/C/Patcher");
    deploy_whdload_file("C/RawDIC", SWITCH_WHDLOAD_ROOT "/C/RawDIC");
    deploy_whdload_file("C/WArc", SWITCH_WHDLOAD_ROOT "/C/WArc");
    deploy_whdload_file("C/WHDLoad.VFS", SWITCH_WHDLOAD_ROOT "/C/WHDLoad.VFS");
    deploy_whdload_file("S/WHDLoad.prefs", SWITCH_WHDLOAD_ROOT "/S/WHDLoad.prefs");
    deploy_whdload_file("S/WHDLoad-Startup", SWITCH_WHDLOAD_ROOT "/S/WHDLoad-Startup");
...
int switch_whdload_prepare_launch(const char *game_name)
{
    ...
    snprintf(startup_dir, sizeof(startup_dir), "%s/S", SWITCH_WHDLOAD_ROOT);
...
int switch_whdload_list(char names[][128], int max_names)
{
    ...
    ensure_directory(SWITCH_WHDLOAD_ROOT);
    DIR *dir = opendir(SWITCH_WHDLOAD_ROOT);
    if (!dir)
        return 0;

    int count = 0;
    struct dirent *entry;
    while (count < max_names && (entry = readdir(dir)) != NULL) {
        if (entry->d_name[0] == '.')
            continue;

        char full[512];
        snprintf(full, sizeof(full), "%s/%s", SWITCH_WHDLOAD_ROOT, entry->d_name);
        struct stat st;
