...
static char s_current_dir[MAX_PATH_LEN] = "./roms";
...
static void scan_directory(const char *path)
{
    ...
    if (dfd) {
        struct dirent *dir;
        while ((dir = readdir(dfd)) != NULL && s_num_entries < MAX_ENTRIES) {
            ...
            if (is_dir || supported) {
                ...
                s_entries[s_num_entries].size = fsz;
                s_num_entries++;
            }
        }
        closedir(dfd);
...
static void try_load_cover(const char *dir_path, const char *filename)
{
    ...
    snprintf(test_paths[2], MAX_PATH_LEN, "./data/covers/%s.png", base_no_ext);
    snprintf(test_paths[3], MAX_PATH_LEN, "./data/covers/%s.jpg", base_no_ext);
    snprintf(test_paths[4], MAX_PATH_LEN, "./data/thumbs/%s.png", base_no_ext);
...
int switch_gui_run_browser(char *out_path, const char *start_dir, int disk_drive_idx)
{
    ...
    while (1) {
        ...
        if (input.pressed & SWITCH_BTN_UP && s_num_entries > 0) {
            selected_idx--;
            if (selected_idx < 0) selected_idx = s_num_entries - 1;
        }
        if (input.pressed & SWITCH_BTN_DOWN && s_num_entries > 0) {
            selected_idx++;
            if (selected_idx >= s_num_entries) selected_idx = 0;
        }

        if (input.held & SWITCH_BTN_UP && s_num_entries > 0) {
            hold_up_counter++;
        ...
        if (input.held & SWITCH_BTN_DOWN && s_num_entries > 0) {
            hold_down_counter++;
        ...
        if (input.pressed & SWITCH_BTN_ZR && s_num_entries > 0) {
            selected_idx += items_per_page;
        ...
        char path_buf[128];
        switch_truncate_text(s_current_dir, SWITCH_SCREEN_W - 220.0f, 0.95f, path_buf, sizeof(path_buf));
        switch_draw_text(80.0f, 60.0f, SWITCH_COLOR_TEXT_WHITE, 0.95f, path_buf);

        char count_str[64];
        snprintf(count_str, sizeof(count_str), "%d files", s_num_entries);
        switch_draw_text_right(SWITCH_SCREEN_W - 36.0f, 60.0f, SWITCH_COLOR_TEXT_MUTED, 0.85f, count_str);
        ...
        for (int i = 0; i < items_per_page; i++) {
            ...
            if (entry_idx < s_num_entries) {
                ...
                switch_draw_card(list_x, item_y, list_w, item_h, is_sel, false);

                char name_buf[128];
                float max_name_w = list_w - 260.0f;
                switch_truncate_text(entry->name, max_name_w, 0.95f, name_buf, sizeof(name_buf));

                if (entry->is_dir) {
                    switch_draw_badge(list_x + 12.0f, item_y + 10.0f, "DIR", RGBA8(40, 55, 80, 255), SWITCH_COLOR_AMIGA_BLUE);
                    switch_draw_text(list_x + 62.0f, item_y + 11.0f, is_sel ? SWITCH_COLOR_TEXT_WHITE : RGBA8(220, 235, 255, 255), 0.95f, name_buf);
                } else {
                    ...
                    switch_draw_text_right(list_x + list_w - 14.0f, item_y + 11.0f, SWITCH_COLOR_TEXT_MUTED, 0.85f, sz_buf);
                }
            }
        }
        ...
        if (selected_idx >= 0 && selected_idx < s_num_entries) {
            ...
            if (s_cover_surf) {
                ...
                switch_draw_rounded_rect(ph_x, ph_y, ph_w, ph_h, 6.0f, RGBA8(18, 22, 32, 255));
                switch_draw_boing_ball_icon(ph_x + (ph_w * 0.5f), ph_y + 80.0f, 32.0f, 0.0f);
                switch_draw_text_centered(ph_x + (ph_w * 0.5f), ph_y + 135.0f, SWITCH_COLOR_TEXT_MUTED, 0.95f, sel_entry->is_dir ? "Folder" : "Amiga Disk Image");
            ...
            if (!sel_entry->is_dir) {
                ...
                switch_truncate_text(size_txt, preview_w - 32.0f, 0.85f, size_buf, sizeof(size_buf));
                switch_draw_text(preview_x + 16.0f, preview_y + 326.0f, SWITCH_COLOR_TEXT_MUTED, 0.85f, size_buf);
            } else {
                switch_draw_text(preview_x + 16.0f, preview_y + 326.0f, SWITCH_COLOR_TEXT_MUTED, 0.85f, "Folder / Subdirectory");
            }
        }
