...
int uae4all_hard_file_ro[4] = { 0, 0, 0, 0 };
...
int gui_init (void)
{
	...
	uae4all_init_sound();
	write_log("[VITA] gui_init: init kickstart\n");
	init_kickstart();
	...
	write_log("[VITA] gui_init: clocks done\n");

	write_log("[VITA] gui_init: launching vita menu\n");
	int initial_menu_result = run_mainMenu_vita();
	disk_sound_reset();
	...
	update_display();
	if (prSDLScreen == NULL) {
		write_log("[VITA] gui_init: game display creation failed, aborting GUI init\n");
		return -1;
	}
...
#elif defined(__SWITCH__)
	emulating=0;
	write_log("[SWITCH] gui_init: init sound\n");
	uae4all_init_sound();
	write_log("[SWITCH] gui_init: init kickstart\n");
	init_kickstart();
	write_log("[SWITCH] gui_init: kickstart done\n");

	write_log("[SWITCH] gui_init: launching switch menu\n");
	int initial_menu_result = run_mainMenu_switch();
	disk_sound_reset();
	write_log("[SWITCH] gui_init: switch menu returned result=%d\n", initial_menu_result);
	if (initial_menu_result == 0) {
		switch_gui_shutdown_final();
		return -2;
	}
	update_display();
	if (prSDLScreen == NULL) {
		write_log("[SWITCH] gui_init: game display creation failed, aborting GUI init\n");
		return -1;
	}
	write_log("[SWITCH] gui_init: game display ready\n");

#ifdef USE_UAE4ALL_VKBD
	vkbd_init();
	write_log("[SWITCH] gui_init: vkbd done\n");
#endif
...
static void goMenu(void)
{
	...
	update_display();
#elif defined(__SWITCH__)
	exitmode = triggerL[0] ? run_overlay_switch() : run_mainMenu_switch();
	if (exitmode == 0) {
		switch_gui_prepare_exit();
		quit_program = 1;
		return;
	}
	update_display();
#elif defined(USE_GUICHAN)
	running=true;
	...
	resume_sound();
#if !defined(USE_GUICHAN) && !defined(__PSP2__)
	...
	if (exitmode==2 || bReloadKickstart)
	{
...
void gui_handle_events (void)
{
	...
	if (buttonB[0]) cd32_button_state |= 2;
	if (buttonX[0]) cd32_button_state |= 4;
	if (buttonY[0]) cd32_button_state |= 8;
	if (triggerL[0]) cd32_button_state |= 16;
	if (triggerR[0]) cd32_button_state |= 32;
