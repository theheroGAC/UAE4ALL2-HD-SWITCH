...
static void draw_dirlist(char *curdir, struct dirent **namelist, int n, int sel)
{
	...
	if (menu_load_type == MENU_LOAD_HD_DIR)
		...
		text_draw_window(x_win,y_win,w_win,h_win,"Insert .ADF, .IPF, .ADZ or .ZIP into DF0 (Triangle = Info)");
	else if (current_drive==1)
		text_draw_window(x_win,y_win,w_win,h_win,"Insert .ADF, .IPF, .ADZ or .ZIP into DF1 (Triangle = Info)");
	else if (current_drive==2)
		text_draw_window(x_win,y_win,w_win,h_win,"Insert .ADF, .IPF, .ADZ or .ZIP into DF2 (Triangle = Info)");
	else if (current_drive==3)
		text_draw_window(x_win,y_win,w_win,h_win,"Insert .ADF, .IPF, .ADZ or .ZIP into DF3 (Triangle = Info)");
#else
#ifdef __SWITCH__
	else if (current_drive==0)
