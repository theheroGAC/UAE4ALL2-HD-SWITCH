...
static void copy_state_path(char *destination, const char *source)
{
	...
	strncpy(destination, source, 254);
	destination[254] = '\0';
}

static void append_state_suffix(char *path, const char *suffix)
{
	if (!path || !suffix) return;
	size_t path_len = strlen(path);
	if (path_len >= 254) return;
	strncat(path, suffix, 254 - path_len);
	path[254] = '\0';
}
...
void make_savestate_filenames(char *save, char *thumb)
{
	...
	if (cdrom_is_cd32_mode() && current_cd_image[0] != '\0')
	{
	...
	if (save[0]=='\0')
	{
		if (uae4all_image_file1[0]!='\0')
			copy_state_path(save, uae4all_image_file1);
...
static bool is_valid_png(const char *path) {
...
void load_savestate_thumbnail() {
	...
	if (test) {
		fclose(test);
		savestate_empty = 0;
		if (is_valid_png(screenshot_filename)) {
